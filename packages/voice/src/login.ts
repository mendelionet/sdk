...
export interface DeviceLoginOptions {
  /** Defaults to https://api.mendelio.net (the /v1/device/code and /oauth/token endpoints live here). */
  apiBaseUrl?: string;
...
export async function deviceLogin(options: DeviceLoginOptions = {}): Promise<{ keyPrefix: string }> {
  ...
  const begin = await doFetch(`${api}/v1/device/code`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ label: hostLabel() }),
  });
  ...
  while (Date.now() < deadline) {
    ...
    const res = await doFetch(`${api}/oauth/token`, {
      method: "POST",
      headers: { "content-type": "application/x-www-form-urlencoded" },
      body: new URLSearchParams({ grant_type: DEVICE_GRANT, device_code: grant.device_code }),
    });
